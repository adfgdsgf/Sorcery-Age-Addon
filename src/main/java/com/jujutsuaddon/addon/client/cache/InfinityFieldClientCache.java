...
// 删除：import net.minecraftforge.api.distmarker.Dist;
// 删除：import net.minecraftforge.api.distmarker.OnlyIn;
...
/**
 * 客户端无下限场数据缓存
 * 修改说明：移除了Client-Only限制，允许服务端加载此类以防止Mixin崩溃。
 * 在服务端运行时，列表将始终为空，逻辑直接返回原向量，不会产生副作用。
 */
public class InfinityFieldClientCache {
    ...
    public static Vec3 modifyMovement(Vec3 playerPos, Vec3 originalMovement) {
        ...
        // 在服务端，fields 永远是空的，所以会直接返回 originalMovement，这是安全的。
        if (fields.isEmpty()) {
